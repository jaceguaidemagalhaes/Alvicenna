...
class PrescriptionDrug {
  ...
  def create(): Unit={
    ...
    try{
      println("Do you want to search drug table? (Y or N)")
      val s = readLine().trim().asInstanceOf[String]
      if(s.toLowerCase() == "y") drug.read()

      println("Drug Id?")
      print("> ")
      drugId = readLine().trim().toInt
      println("Instructions?")
      print("> ")
      instructions = readLine().trim()

      query = (s"INSERT INTO prescriptionDrug (prescriptionId, drugId, instructions)" +
        s""" VALUES(${Main.defaultPrescription}, $drugId, "$instructions")""")
      executeQuery = new ExecuteQuery(query, false)
  ...
  def read(p_tableId: Int = 0): Unit={
    ...
    try{
      ...
      try while ( {
        ...
        prescriptionDrugId = executeQuery.resultSet.getInt("prescriptionDrugId")
        ...
        objectMap += (prescriptionDrugId -> (prescriptionId,drugId,instructions))
        drug.read(drugId)
        println("ID: " + prescriptionDrugId)
        println("Drug: " + drug.dataMap(drugId)._1)
        println("Doctor's Instructions: " + instructions)
      }
